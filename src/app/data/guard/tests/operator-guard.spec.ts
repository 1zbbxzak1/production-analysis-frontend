...
describe('operatorGuard', (): void => {
    ...
    it('should return false and redirect to home when user does not have Operator role', (done: DoneFn): void => {
        mockAuthManagerService.getAccessToken.and.returnValue('valid-token');
        mockAuthManagerService.getUserRoles.and.returnValue(['DepartmentHead', 'Admin']);
    ...
    it('should return true when user has Operator role among multiple roles', (done: DoneFn): void => {
        mockAuthManagerService.getAccessToken.and.returnValue('valid-token');
